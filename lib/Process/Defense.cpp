...
void Defense::get_vector(void){
    ...
    line_detect = (line_dist*10 == 999) ? false : true;// ラインが検出されているかどうか
    ...
    line_power = 120/(11-line_dist);//距離でパワーを調整
...
void Defense::p4(void){//ラインに戻る
    ...
    mymotor.run(lastdetect,backPower*motor_rate,0);//最後の検出角度・戻るパワー・０度
