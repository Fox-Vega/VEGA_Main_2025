...
void Attack::attack_() { //ディフェンス
    if (line.get_type() != 0) { //ラインある
        if (line.get_type() == 1) {
            waypoint1 = (line.get_pack(0) + 270) % 360;
            waypoint2 = (line.get_pack(0) + 90) % 360;
        } else if (line.get_type() == 2) {
            waypoint1 = line.get_pack(0);
            waypoint2 = line.get_pack(1);
        } else {
            //後で
        }
        int waypointerror1 = (waypoint1 - ball.get_azimuth() + 360) % 360;
        int waypointerror2 = (waypoint2 - ball.get_azimuth() + 360) % 360;

        if (waypointerror1 < waypointerror2) {
            mymotor.run(waypoint1, 200, 0);
        } else {
            mymotor.run(waypoint2, 200, 0);
        }
    } else if (ball.get_stat() == 1) { //ラインない　ボールある
}